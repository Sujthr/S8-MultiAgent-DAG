                ...
                if result.success:
                    ...
                    if graph.g.nodes[nid]["skill"] == "formatter":
                        ...
                        if isinstance(fa, str) and fa.strip():
                            formatter_answer = fa
                            # Answer obtained — cancel pending/running sibling
                            # nodes so the loop exits instead of waiting on
                            # nodes added by recovery planners.
                            for other in graph.g.nodes:
                                if graph.g.nodes[other]["status"] in ("pending", "running"):
                                    graph.mark(other, "skipped")
                else:
