...
async def chat(req: ChatRequest):
    ...
    # Key-family rotation: "gemini" expands to [gemini, gemini1, ..., gemini5]
    # so quota-exhausted keys fall through to the next key automatically.
    _agent_family: list[str] = []
    if req.agent and not req.provider:
        pinned = AGENT_ROUTING.get(req.agent)
        if pinned and pinned in router.providers:
            base = pinned.rstrip("0123456789")
            _agent_family = [pinned] + [
                f"{base}{i}" for i in range(1, 6)
                if f"{base}{i}" != pinned and f"{base}{i}" in router.providers
            ]
            req.provider = pinned
            # Allow fallback within the key family; true explicit_override is
            # reserved for caller-specified provider= only.
            explicit_override = len(_agent_family) == 1
    ...
    if req.auto_route and not req.provider:
        ...
        # Key family first, then remaining providers ordered by capability
        # (strong models before small ones) as final fallback.
        family_set = set(_agent_family)
        family_candidates = [p for p in _agent_family if p in router.providers]
        _quality_order = ["groq", "cerebras", "openrouter", "github", "nvidia", "ollama"]
