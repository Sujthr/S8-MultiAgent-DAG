...
def build_providers(cache_store):
    ...
    """
    out = {}
    _gem_model = os.getenv("GEMINI_MODEL", "gemini-2.5-flash")
    if k := os.getenv("GEMINI_API_KEY"):
        out["gemini"] = GeminiProvider(k, _gem_model, cache_store)
    # Extra Gemini keys (gemini1..gemini5) for parallel fan-out — each key has
    # its own independent quota so concurrent researchers don't share a limit.
    for _slot in range(1, 6):
        _env = f"GEMINI_API_KEY_{_slot}"
        if (_k := os.getenv(_env)) and _k.startswith("AIzaSy"):
            _name = f"gemini{_slot}"
            if _name not in out:  # don't duplicate the primary key
                out[_name] = GeminiProvider(_k, _gem_model, cache_store)
